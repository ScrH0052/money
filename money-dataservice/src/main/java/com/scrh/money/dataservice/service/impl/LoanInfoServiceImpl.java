...
public class LoanInfoServiceImpl implements LoanInfoService {
    ...
    public Map<String, Object> queryLoanProductByTypeCode(Integer targetPage, Integer type) {
        ...
        //查询页面信息
        Integer totalCount = (Integer) redisTemplate.opsForValue().get("totalCountOfLoan" + type);
        if (null == totalCount) {
            totalCount = loanInfoMapper.selectCountOfLoanProductByTypeCode(type);
            redisTemplate.opsForValue().set("totalCountOfLoan" + type, totalCount, Duration.ofMinutes(5));
        }
